...
public class LoanAccount extends Account {
    ...
    public void payEMI() {
        if (getBalance() >= emiAmount) {
            ...
            System.out.println("EMI of " + emiAmount + " paid for loan account " );
        } else {
            System.out.println("Insufficient balance to pay EMI for loan account " );
        }
    }

    public void topUpLoan(float amount) {
        loanAmount += amount;
        float curAmount = getBalance();
        setBalance(curAmount + amount);
        System.out.println("Loan amount topped by " + amount );
    }
